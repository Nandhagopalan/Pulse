...
function SignedChart({ title, badge, vals }: { title: string; badge: string; vals: number[] }) {
  const h = barsSigned(vals, 300, 96, 6);
  return (
    <Card style={{ padding: '18px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: T.text }}>{title}</div>
        <div style={{ fontSize: 11, fontWeight: 600, color: T.muted, background: T.borderSoft, borderRadius: 99, padding: '3px 10px' }}>{badge}</div>
      </div>
...
function FlowsChart({ fii, dii }: { fii: number[]; dii: number[] }) {
  ...
  const lastF = fii[fii.length - 1], lastD = dii[dii.length - 1];
  return (
    <Card style={{ padding: '18px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 8, fontSize: 11, color: T.muted }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 9, height: 9, borderRadius: 2, background: T.amber }} />FII</span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 9, height: 9, borderRadius: 2, background: T.navy }} />DII</span>
        <span style={{ color: T.faint }}>last 20 sessions · provisional</span>
      </div>
    </Card>
  );
}

interface MetricSpec { id: string; label: string; color: string; series: number[]; def: CardMode; barStyle: 'bottom' | 'top' | 'signed'; span: number }

export function BreadthTab({ D, cardMode, setCard }: {
  D: MarketData;
  ...
  const specs: MetricSpec[] = [
    { id: 'newHighs', label: 'New Highs', color: T.up, series: D.series.newHighs, def: 'bar', barStyle: 'bottom', span: 4 },
    { id: 'newLows', label: 'New Lows', color: T.down, series: D.series.newLows, def: 'bar', barStyle: 'top', span: 4 },
    { id: 'netHL', label: 'Net Highs − Lows', color: T.up, series: D.series.netHL, def: 'bar', barStyle: 'signed', span: 4 },
    { id: 'up20', label: 'Up 20%+ in 5D', color: T.navy, series: D.series.up20, def: 'area', barStyle: 'bottom', span: 6 },
    { id: 'up30', label: 'Up 30%+ in 5D', color: T.navy, series: D.series.up30, def: 'area', barStyle: 'bottom', span: 6 },
    { id: 'up4vol', label: 'Up 4%+ on Volume', color: T.up, series: D.series.up4vol, def: 'area', barStyle: 'bottom', span: 6 },
    { id: 'down4vol', label: 'Down 4%+ on Volume', color: T.down, series: D.series.down4vol, def: 'area', barStyle: 'bottom', span: 6 },
  ];
  ...
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1.05fr 1.45fr', gap: 16, marginTop: 18 }}>
        <Card style={{ padding: '22px 24px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: T.text, background: T.borderSoft, borderRadius: 99, padding: '5px 12px' }}>+20% in 5D · <Mono size={12} color={T.up} weight={600}>{D.series.up20[44]}</Mono></div>
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: T.text, background: T.borderSoft, borderRadius: 99, padding: '5px 12px' }}>A/D 2 up days</div>
          </div>
        </Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginTop: 16 }}>
        <SignedChart title="Daily Advances − Declines" badge={'latest ' + (D.adDaily[89] >= 0 ? '+' : '') + D.adDaily[89]} vals={D.adDaily} />
        <SignedChart title="Net Highs − Lows" badge={'NNH ' + (D.nhDaily[89] >= 0 ? '+' : '') + D.nhDaily[89]} vals={D.nhDaily} />
        <FlowsChart fii={D.flows.fii} dii={D.flows.dii} />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginTop: 26 }}>
        ...
        <div style={{ fontSize: 12, color: T.faint }}>daily counts across the universe</div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 14, marginTop: 14 }}>
        {specs.map(spec => {
          ...
          const value = (spec.id === 'netHL' && last >= 0 ? '+' : '') + Math.round(last);
          let area = '', line = '', bars = '', barsNeg = '', baseline = '';
          if (m === 'area') {
            const p = areaLine(spec.series, 300, 104, 8, 10); area = p.area; line = p.line;
          } else if (spec.barStyle === 'signed') {
            const b = barsSigned(spec.series, 300, 104, 8); bars = b.pos; barsNeg = b.neg; baseline = 'M0 ' + b.zy + ' H300';
          } else if (spec.barStyle === 'top') {
            bars = barsTop(spec.series, 300, 104, 10);
          } else {
            bars = barsBottom(spec.series, 300, 104, 10);
          }
          const valueColor = spec.id === 'netHL' ? (last >= 0 ? T.up : T.down) : spec.color;
